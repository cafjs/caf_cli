...
// we want to speed up first retries for fast authentication
var TIMEOUT_DIVIDER = 10;
    ...
    if (current < max) {
        var cb0 = function(err, value) {
            if (err) {
                ...
                var actualTimeout = (timeout * current) /TIMEOUT_DIVIDER;
                actualTimeout = (actualTimeout > timeout ? timeout :
                                 actualTimeout);
                setTimeout(function() {
 ...
 *
 */
var HTTPRequest = function(url, msg, cookieJar, proxy) {
    this.url = url;
    this.msg = msg;
...
//    http.globalAgent.maxSockets = spec.maxSockets || 100;
    this.url = spec.url;
    this.hostname = url_lib.parse(this.url).hostname;
...
Channel.prototype.invokeAsync = function(req, cb) {
    ...
    var checkLocal = function(cb0) {
        var matchIP = function(intf, ip) {
            var result = false;
            Object.keys(intf)
    ...
    async.series([
                     function(cb0) {
                         if (self.proxy &&
                             (typeof self.isLocal !== 'boolean')) {
                             // ensure local addresses are not proxied
                             checkLocal(cb0);
                         } else {
                             cb0(null);
                         }
                     ...
                     function(cb0) {
                         doAtMostNTimes(f, self.maxRetries, self.retryTimeout,
                                        cb0);
                     }
                 ], function(err, data) {
                     if (err) {
