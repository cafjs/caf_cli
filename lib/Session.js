            ...
            .forEach(function(x) {
                         that[x] = that[x] || function() {
                             ...
                             if ((args.length > 0) &&
                                 ...
                                 if (multi === null) {
                                     queues.rpc.remoteInvoke(webSocket, x,
                                                             meta[x], args);
                                 } else {
                                     var f = args.pop();
                                     multi.push({method: x, meta : meta[x],
                                                 args: args});
                                     queues.rpc.remoteInvoke(webSocket,
                                                             MULTI_METHOD,
                                                             meta[MULTI_METHOD],
                                                             [multi, f]);
                                     multi = null;
 ...
 *
 */
var extractTokenFromURL = exports.extractTokenFromURL = function(urlStr) {
    var token = null;
 ...
 * @param {caOwner: string, caLocalName: string, appPublisher: string,
 *  appLocalName: string, token: string, cacheKey: string} props
 * @return {string} A patched URL.
 *
 */
var patchURL = exports.patchURL = function(url, props) {
