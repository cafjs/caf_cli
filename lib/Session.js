 ...
 *      cacheKey: string, initUser: boolean,
 *      log: function(string),
...
var TimeAdjuster = require('./TimeAdjuster').TimeAdjuster;
...
exports.Session = function(url, caId, options) {
    ...
    options.newToken = options.newToken || function(msg, cb) {
        try {
            ...
            if (root && root.location && root.location.href) {
                ...
                // node.js client
                var tokenF = TokenFactory(options);
    ...
    var addMethods = function(meta) {
        ...
        that.getPromise = function() {
            if (multi === null) {
                var err = new Error('No methods to call');
                throw err;
            } else {
                return new Promise(function(resolve, reject) {
                    ...
                    if (multi.length === 1) {
                        // normal call
                        var method = multi[0].method;
                        var args = multi[0].args;
                        args.push(cb);
                        multi = null;
    ...
    var handleMsg = function(msg) {
        if (json_rpc.isSystemError(msg)) {
            if (json_rpc.isRedirect(msg)) {
                ...
                options.log(msg);
                var err = new Error('Non-recoverable error');
                err['msg'] = msg;
                that.close(err);
            }
            return true;
...
exports.extractSpecFromURL = function(urlStr) {
    ...
    if (!myId) {
        var err = new Error("Missing 'from' field in URL");
        err['url'] = urlStr;
        throw err;
    }
    var appProtocol = (parsedURL.protocol === 'http:' ? 'http' : 'https');
    var hostSplit = parsedURL.host.split('.');
    var name = json_rpc.splitName(hostSplit.shift());
    var appPublisher = name[0];
    var appLocalName = name[1];
    var appSuffix = hostSplit.join('.');
    return {
