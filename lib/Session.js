...
var Session = exports.Session = function(spec) {
    ...
    this.reqChannel
        .on('error', function(err) {
                self.disconnectHandler(self.reqChannel, err);
            })
        .on('badToken', function(lastToken) {
                self.newLogin(self.reqChannel, lastToken);
            });
    ...
    if (!(this.disableBackChannel)) {
        ...
        this.backChannel
            .on('error', function(err) {
                    self.disconnectHandler(self.backChannel, err);
                })
            .on('notified', function(notif) {
                    self.notificationHandler(self.backChannel, notif);
                })
            .on('badToken', function(lastToken) {
                    self.newLogin(self.backChannel, lastToken);
                });
    ...
    this.loginChannel
        .on('error', function(err) {
                self.disconnectHandler(self.loginChannel, err);
            })
        .on('badToken', function(lastToken) {
                // password failed
                self.badLogin(lastToken, 'BadToken emitted in login channel');
            });
...
Session.prototype.badLogin = function(lastToken, info) {
    ...
    if (lastToken === this.token) {
        self.disconnectHandler(self.loginChannel, 'Authorization failed');
    }
};

Session.prototype.newLogin = function(channel, lastToken) {
    var self = this;
...
Session.prototype.disconnectHandler = function(channel, err) {
...
Session.prototype.notificationHandler = function(channel, notif) {
    if (this.cbObject) {
        var cb = function(err) {
            if (err) {
