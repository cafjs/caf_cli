...
/**
 * Adds a fragment to a URL with some metadata.
 *
 * @param {string} url A URL to patch.
 * @param {string=} caOwner
 * @param {string=} caLocalName
 * @param {string=} token
 * @return {string} A patched URL.
 *
 */
var patchURL = exports.patchURL = function(url, caOwner, caLocalName, token) {
    var options = {};
    var parsedURL = urlParser.parse(url);
    if (parsedURL.hash && (parsedURL.hash.indexOf('#') === 0)) {
    ...
    if (caOwner && caLocalName) {
        var caName = caOwner + json_rpc.NAME_SEPARATOR  + caLocalName;
        options.ca = caName;
        options.from = caName;
    }
    if (token) {
